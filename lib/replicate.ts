// lib/replicate.ts — Image generation via Replicate (FLUX Schnell)
//
// Per-page illustrations use FLUX Schnell with the character's full storyDescription
// injected into every page prompt + a fixed seed per book for style consistency.
//
// Why not fofr/consistent-character?
//   That model uses InstantID which requires a human face in the subject image.
//   Our characters are cartoon creatures — InstantID throws "No face detected"
//   and returns output: [] every time.
//
// Cost: FLUX Schnell ~$0.003/image × 14 pages ≈ $0.04/book
...
// Used by scripts/generate-anchors.ts to produce character reference images
...
function ageProportions(age: number): string {
  ...
  return `${p.height}. Body: ${p.build}. Face and head: ${p.face}.`
}
...
// ── Anchor generation (used by scripts/generate-anchors.ts only) ─────────────
...
//
// Uses FLUX Schnell. The character's storyDescription is injected into every
// page prompt so the model "knows" what the creature looks like on every page.
// seed is fixed per book so the art style stays consistent across pages.
...
export async function generatePageImage(
  ...
  const prompt = buildPagePrompt(pageImagePrompt, characterDescription)
  ...
  try {
    ...
    throw new ImageGenerationError('Replicate FLUX Schnell request failed', err)
  }
  ...
  const imageUrl = Array.isArray(output) ? output[0] : output
  if (typeof imageUrl !== 'string' || !imageUrl) {
    throw new ImageGenerationError('FLUX Schnell returned no image URL: ' + JSON.stringify(data))
  }
